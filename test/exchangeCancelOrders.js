...
describe("Exchange_Cancel_Orders", function () {
  ...
  describe("Cancel_One", () => {
    it("Should cancel a valid order", async function () {
      ...
      let isValid = await flowExchange.isNonceValid(signer2.address, nonce);
      expect(isValid).to.be.true;

      // cancel order
      await flowExchange.connect(signer2).cancelMultipleOrders([nonce]);

      // invalid after cancel
      isValid = await flowExchange.isNonceValid(signer2.address, nonce);
      ...
      await approveERC20(signer1.address, execParams[1], salePrice, signer1, flowExchange.address);
      ...
      await expect(
        flowExchange.connect(signer1).takeOrders([sellOrder], [buyOrder.nfts])
      ...
      // balance after sale
      expect(await token.balanceOf(flowExchange.address)).to.equal(0);
      expect(await token.balanceOf(signer1.address)).to.equal(INITIAL_SUPPLY.div(2));
      expect(await token.balanceOf(signer2.address)).to.equal(INITIAL_SUPPLY.div(2));
    });
  });
  ...
  describe("Cancel_Multiple", () => {
    it("Should cancel multiple valid orders", async function () {
      ...
      let isValid = await flowExchange.isNonceValid(signer2.address, nonce1);
      expect(isValid).to.be.true;
      isValid = await flowExchange.isNonceValid(signer2.address, nonce2);
      expect(isValid).to.be.true;

      // cancel orders
      await flowExchange.connect(signer2).cancelMultipleOrders([nonce1, nonce2]);

      // invalid after cancel
      isValid = await flowExchange.isNonceValid(signer2.address, nonce1);
      expect(isValid).to.be.false;
      isValid = await flowExchange.isNonceValid(signer2.address, nonce2);
      expect(isValid).to.be.false;
      ...
      await expect(
        flowExchange.connect(signer2).cancelMultipleOrders([nonce1, nonce2])
      ).to.be.revertedWith("nonce already exec or cancelled");
    });
  });
  ...
  describe("Cancel_AfterOrderExecution", () => {
    it("Should not cancel already executed order", async function () {
      ...
      await flowExchange
        .connect(signer1)
        .takeOrders([sellOrder], [buyOrder.nfts], { value: salePrice });
      ...
      expect(await ethers.provider.getBalance(flowExchange.address)).to.equal(totalFees);
      ...
      // try canceling
      await expect(flowExchange.connect(signer2).cancelMultipleOrders([nonce])).to.be.revertedWith(
  ...
  describe("Cancel_AllOrders", () => {
    it("Should cancel all orders", async function () {
      ...
      await expect(flowExchange.connect(signer2).cancelAllOrders(100001)).to.be.revertedWith(
        "too many"
      );

      // cancel all orders
      await flowExchange.connect(signer2).cancelAllOrders(minCancelNonce);
      // min order nonce should be 100
      let newMinOrderNonce = await flowExchange.userMinOrderNonce(signer2.address);
  ...
  describe("Post_Cancel_AllOrders_Higher_Nonces", () => {
    it("Should cancel higher nonces", async function () {
      ...
      await expect(
        flowExchange.connect(signer2).cancelMultipleOrders([minCancelNonce + 1, minCancelNonce + 2])
      ).to.not.be.revertedWith("nonce too low");
      // min order nonce should still be 100
      let newMinOrderNonce = await flowExchange.userMinOrderNonce(signer2.address);
      expect(newMinOrderNonce).to.equal(minCancelNonce);

      await flowExchange.connect(signer2).cancelAllOrders(minCancelNonce + 1);
      newMinOrderNonce = await flowExchange.userMinOrderNonce(signer2.address);
      expect(newMinOrderNonce).to.equal(minCancelNonce + 1);

      await flowExchange.connect(signer2).cancelAllOrders(minCancelNonce + 3);
      newMinOrderNonce = await flowExchange.userMinOrderNonce(signer2.address);
